...
if(_exists) then {
      ["EXISTS"] remoteExec ["Client_fnc_createProfile_action", _player];
} else {
      private _id = player getVariable["core_user_id", -1];
      private _uid = format["AZ%1", [7, "ABCDEFGHIJKLMNOPQRSTUVWXYZ"] call Client_fnc_generateString];
      if(_id != -1) then {
            [format["core_insert_profile:%1:%2:%3:%4:%5", _id, _uid, _first_name, _last_name, _female], 0] call ExternalS_fnc_ExtDBquery;
      };
};
