...
if(_from_profile != _to_profile) then {
      ...
      if(!isNull _to_obj) then {
            [["STR_CORE_ATM_ACTION_TRANSFER_RECEIVE", _amount, _from_acc, _to_acc], true] remoteExecCall ["Client_fnc_domsg", _to_obj];
      };
      if(!isNull _from_obj) then {
            [["STR_CORE_ATM_ACTION_TRANSFER_SEND", _amount, _to_acc, _from_acc], true] remoteExecCall ["Client_fnc_domsg", _from_obj];
      };
} else {
      private _from_obj = [_from_profile] call Client_fnc_getPlayerByProfile;
      if(!isNull _from_obj) then {
            [["STR_CORE_ATM_ACTION_TRANSFER_BETWEEN", _amount, _from_acc, _to_acc], true] remoteExecCall ["Client_fnc_domsg", _from_obj];
      };
};
