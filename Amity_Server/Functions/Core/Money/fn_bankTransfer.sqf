...
if(_from_profile != _to_profile) then {
      ...
      if(!isNull _to_obj) then {
            [format["You have been wire transfered $%1 from account(%2) to your bank account(%3)", _amount, _from_acc, _to_acc], true] remoteExec ["Client_fnc_domsg", _to_obj];
      };
      if(!isNull _from_obj) then {
            [format["You have wire transfered $%1 to account(%2) from your bank account(%3)", _amount, _to_acc, _from_acc], true] remoteExec ["Client_fnc_domsg", _from_obj];
      };
} else {
      private _from_obj = [_from_profile] call Client_fnc_getPlayerByProfile;
      if(!isNull _from_obj) then {
            [format["You have wire transfered $%1 between your accounts.", _amount, _from_acc, _to_acc], true] remoteExec ["Client_fnc_domsg", _from_obj];
      };
};
