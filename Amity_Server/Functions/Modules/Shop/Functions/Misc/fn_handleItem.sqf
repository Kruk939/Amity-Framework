            ...
            if(_id == _category_id) exitWith {
                  {
                        ...
                        if(_id == _item_id) exitWith {
                              if((_stock + _count) >= 0) then {
                                    private _stock = 0;
                                    if(_count < 0) then {
                                          private _q = [format["shop_update_items_count_sub:%1:%2", _id, (0 - _count)], 2] call ExternalS_fnc_ExtDBasync;
                                          _stock = (_q select 0) select 0;
                                    } else {
                                          private _q = [format["shop_update_items_count_add:%1:%2", _id, _count], 2] call ExternalS_fnc_ExtDBasync;
                                          _stock = (_q select 0) select 0;
                                    };
                                    _x set[4, _stock];
                                    _ret = _x;
