...
if(_type == "ACCEPT") exitWith {
      ...
      [player, _target,[_amount, _reason, _points, _preset]] remoteExecCall["ServerModules_Ticket_fnc_insertTicket", 2];
      closeDialog 0;
};
if(_type == "EXIT") exitWith {
      [["STR_TICKET_ACTION_REFUSED", player getVariable["name", ""]], true] remoteExecCall["Client_fnc_domsg", _target];
      closeDialog 0;
};
