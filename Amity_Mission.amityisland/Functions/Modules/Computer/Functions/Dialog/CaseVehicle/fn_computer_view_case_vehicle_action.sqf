...
if(_type == "SAVE") exitWith {
      ...
      [_id, _wID, true] remoteExec["ServerModules_Computer_fnc_caseUpdateLevel", 2];
      [] call ClientModules_Computer_fnc_openLast;
};
if(_type == "CLOSE") exitWith {
      _data params["", "", "", "", "", "_closed_by", "", "", ""];
      if(typeName _closed_by == "OBJECT") then {
            [_id, player getVariable["profile_id", -1], true] remoteExec["ServerModules_Computer_fnc_caseSetStatus", 2];
      } else {
