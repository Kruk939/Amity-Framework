...
private _config = switch(_type) do {
      case "shop": { missionConfigFile >> "Shop"};
      default {
            configNull;
      };
};
...
_marker setMarkerTextLocal  (getText(_config >> "Notify" >> "message")) call BIS_fnc_localize;
private _time = getNumber(_config >> "Marker" >> "time");
[_name, _time] spawn {
      params["_name", "_time"];
      uiSleep _time;
