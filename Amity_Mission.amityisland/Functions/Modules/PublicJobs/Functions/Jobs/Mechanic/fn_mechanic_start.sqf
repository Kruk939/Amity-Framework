...
[] spawn {
      uiSleep 5;
      private _minDistance = getNumber(missionConfigFile >> "PublicJobs" >> "Jobs" >> "Mechanic" >> "Setup" >> "minDistance");
      while{public_jobs_var_running} do {
            private _object = public_jobs_mechanic_var_objects call BIS_fnc_selectRandom;
            if(!(_object getVariable["amity_repair", false]) && _object distance player > _minDistance && (damage _object) == 1) then {
                  [localize "STR_PUBLIC_JOBS_MECHANIC_MAP_MARKER", getPos _object] call ClientModules_PublicJobs_fnc_addMarker;
